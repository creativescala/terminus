...
final class Text(
  ...
  def maxIntrinsicHeight(width: Int | Infinity)(using Observe): Int =
    ...
    val lineCount =
      width match
        case Infinity => value.get.lines.length
        case w: Int   =>
          value.get.reflow((w - insets.horizontal).max(0)).length
  ...
  def render(dimensions: Dimensions, buf: Buffer)(using Observe): Unit =
    ...
    var row = 0
    value.get.reflow(inner.width).foreach { line =>
      textBuf.putLine(0, row, line, ac)
      row = row + 1
    }

  /** The width of the widest logical (unwrapped) line of content, in cells. */
  private def naturalContentWidth(using Observe): Int =
    value.get.lines.map(_.width).maxOption.getOrElse(0)

  private def activeProps(using Observe): TextProps =
    style(context.state)
object Text:
