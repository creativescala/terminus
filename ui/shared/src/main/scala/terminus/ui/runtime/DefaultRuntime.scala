...
/** The runtime provides capabilties that are internal to the system and not
  ...
  * mutable state.
  */
final class DefaultRuntime(runner: Runner) extends Runtime, Schedule:
  ...
  def quit(): Unit = shouldQuit = true

  def quitRequested: Boolean = shouldQuit
  ...
  private val focusables: mutable.Map[FocusId, DefaultRuntime.Focusable] =
    mutable.Map.empty
  ...
  def schedule(stream: Stream[IO, Runnable]): Unit =
    runner.schedule(stream)

  def schedule(effect: Effect): Unit =
    runner.schedule(IO.pure(effect))
  ...
  def addKeyHandler(
    ...
    focusables
      .getOrElseUpdate(focusId, DefaultRuntime.Focusable.empty)
