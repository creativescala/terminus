...
/** Typeclass for handling the messiness of reading, updating, and writing termios structs in a platform
  * specific way.
  *
  * Unfortunately, while all POSIX operating systems have `termios.h` defined, there `termios` structure
  * is inconsistent in the size of the bitflags used to hold the terminal settings. As an example, Linux
  * uses CInt to hold bitflags while OSX uses CLong.
  *
  * At the time of this writing, the Scala Native implementation does not currently handle OS specific
  * differences in
  *
  * @see [[https://github.com/scala-native/scala-native/issues/4143 Scala Native termios linux issue]]
  *
  * @tparam T The type of the termios structure, should be on of either [[TermiosStruct.cint_flags]] or
  *           [[TermiosStruct.clong_flags]]. At the time of writing this documentation, these are the
  *           only known variants of the termios struct. If more are found in the future, they should
  *           be added.
  */
@implicitNotFound("There is no terminus.TermiosAccess instance defined for the given termios struct.")
trait TermiosAccess[T] {
  ...
  /** Allocates a new termios structure defined by [[this.T]], in the given [[Zone]] and copies the
    * current terminal settings into it, returning a pointer in memory to this structure.
    */
  def get(using Zone): Ptr[T]

  /** Sets the terminal settings to the attributes defined by the termios structure stored at the
    * pointer specified by the [[ptr]] argument.
    */
...
given clongTermiosAccess: TermiosAccess[TermiosStruct.clong_flags] =
  new TermiosAccess[TermiosStruct.clong_flags] {
    ...
    override def set(ptr: Ptr[TermiosStruct.clong_flags]): Unit = {
      val _ = posix.termios.tcsetattr(STDIN_FILENO, posix.termios.TCSAFLUSH, ptr)
      ()
    }

    override def addInputFlags(attrs: Ptr[TermiosStruct.clong_flags], flags: CInt): Unit =
      attrs._1 = attrs._1 | flags

    override def removeInputFlags(attrs: Ptr[TermiosStruct.clong_flags], flags: CInt): Unit =
      attrs._1 = attrs._1 & ~flags

    override def addOutputFlags(attrs: Ptr[TermiosStruct.clong_flags], flags: CInt): Unit =
      attrs._2 = attrs._2 | flags

    override def removeOutputFlags(attrs: Ptr[TermiosStruct.clong_flags], flags: CInt): Unit =
      attrs._2 = attrs._2 & ~flags

    override def addControlFlags(attrs: Ptr[TermiosStruct.clong_flags], flags: CInt): Unit =
      attrs._3 = attrs._3 | flags

    override def removeControlFlags(attrs: Ptr[TermiosStruct.clong_flags], flags: CInt): Unit =
      attrs._3 = attrs._3 & ~flags

    override def addLocalFlags(attrs: Ptr[TermiosStruct.clong_flags], flags: CInt): Unit =
      attrs._4 = attrs._4 | flags

    override def removeLocalFlags(attrs: Ptr[TermiosStruct.clong_flags], flags: CInt): Unit =
      attrs._4 = attrs._4 & ~flags
  }
...
given cintTermiosAccess: TermiosAccess[TermiosStruct.cint_flags] =
  new TermiosAccess[TermiosStruct.cint_flags] {
    ...
    override def addInputFlags(attrs: Ptr[TermiosStruct.cint_flags], flags: CInt): Unit =
      attrs._1 = attrs._1 | flags

    override def removeInputFlags(attrs: Ptr[TermiosStruct.cint_flags], flags: CInt): Unit =
      attrs._1 = attrs._1 & ~flags

    override def addOutputFlags(attrs: Ptr[TermiosStruct.cint_flags], flags: CInt): Unit =
      attrs._2 = attrs._2 | flags

    override def removeOutputFlags(attrs: Ptr[TermiosStruct.cint_flags], flags: CInt): Unit =
      attrs._2 = attrs._2 & ~flags

    override def addControlFlags(attrs: Ptr[TermiosStruct.cint_flags], flags: CInt): Unit =
      attrs._3 = attrs._3 | flags

    override def removeControlFlags(attrs: Ptr[TermiosStruct.cint_flags], flags: CInt): Unit =
      attrs._3 = attrs._3 & ~flags
